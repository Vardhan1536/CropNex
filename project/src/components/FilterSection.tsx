  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Raw suggestions payload:", data.suggestions);


        if (Array.isArray(suggestionsPayload)) {
          // If the backend sends suggestions AND includes message inside them, filter them out
          const validSuggestions = suggestionsPayload.filter(s => s.market_name); // Or use a more reliable field

          if (validSuggestions.length > 0) {
            setSuggestionResult(validSuggestions);
          } else {
            // Could be just a message with no real suggestions
            const first = suggestionsPayload[0];
            if (first?.message) {
              setMessage(first.message);
              setModalType('info');
              setIsModalOpen(true);
            } else {
              setMessage('No valid market suggestions were found.');
              setModalType('info');
              setIsModalOpen(true);
            }
          }
